        ...
        '$http', '$q', function($http, $q) {
            return {
                list: function(type, offset, size) {
                    var defer = $q.defer();
                    $http.get('http://localhost:3000/songlist?type=' + type + '&size=' + size + '&offset=' + offset, {
                        cache: true
                ...
                search: function(keyword) {
                    var defer = $q.defer();
                    $http.get('http://localhost:3000/search?keyword=' + keyword).success(function(data) {
                        defer.resolve(data);
                    }).error(function(err) {
                        defer.reject(err);
                    });
                ...
                base: function(id) {
                    var defer = $q.defer();
                    $http.get('http://localhost:3000/songid?songid=' + id).success(function(data) {
                        if (data.error) {
                ...
                lrc: function(song_id) {
                    var defer = $q.defer();
                    $http.get('http://localhost:3000/songlrc?songlrcId=' + song_id, {
                        cache: true
                    }).success(function(data) {
                        if (data.error) {
                            defer.reject(data.error);
        ...
        '$scope', '$timeout', function(MusicService, $rootScope, $http, $scope, $timeout) {
            ...
            $scope.load = function(item, force) {
                ...
                if ((force != undefined && force) || item.id != $scope.song.id) {
                    //加载歌词，歌曲图片，作者，歌曲名称，
                    console.log("我要播放的第一首歌是:" + item.title);
